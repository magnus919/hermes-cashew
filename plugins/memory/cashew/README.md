...
Configured via `cashew.json` in the active Hermes home (`HERMES_HOME`, default
`~/.hermes`), auto-generated by `hermes memory setup`.
...
postcondition, rollback, and mutation uncertainty (`mutation_uncertain`) outcomes
to the operator.

## Logs

Plugin logs go to `logs/agent.log` under the active Hermes home (not gateway.log — the gateway has a component filter).
