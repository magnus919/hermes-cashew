...
def test_e2e_install_lifecycle_schema_returns_field_descriptors(tmp_path):
    ...
    for i, field in enumerate(schema):
        assert isinstance(field, dict), (
            f"Field #{i} must be dict, got {type(field).__name__}"
        )
    ...
    assert defaults_by_key["embedding_device"] == "cpu"
...
def test_e2e_full_lifecycle_with_retrieval_and_sync(tmp_path):
    ...
    try:
        ...
        assert "E2E test content" in context, (
            f"prefetch should find seeded node, got: {context!r}"
        )
