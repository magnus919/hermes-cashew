...
replace it with an immutable canonical revision or release after independently
verifying the same contracts. PyPI publication separately requires a suitable
release because direct URL dependencies are rejected:
...
path, so the transition shim was removed. Read-only integrity inspection does
not mutate the database; the separate repair action is opt-in, backup-backed,
and delegates to upstream only after explicit confirmation.
...
| Standalone config schema and auxiliary LLM resolution [`config.py`](../plugins/memory/cashew/config.py) | Hermes owns `config.yaml` and auxiliary roles; the plugin persists its own 17-field `cashew.json`, translates environment overrides, constrains paths, and resolves the configured `auxiliary.<role>` callable. This persistence remains necessary for setup and cron subprocesses. | The adapter calls Hermes's supported `agent.auxiliary_client.get_text_auxiliary_client(role)` inside a bounded worker after confirming the explicit role mapping. It retains prompt/response bounds and heuristic fallback at the provider boundary. | **Retain current split.** Hermes owns client/model routing; the adapter owns Cashew configuration persistence, bounded invocation, and failure isolation. Auxiliary-interface, config-roundtrip, profile-isolation, and loader tests cover the current contract. |
...
| --- | --- | --- |
| CPU/model selection and embedding dimensions | `uv run --frozen --extra dev pytest tests/test_embedding_device.py tests/test_migration.py -q` | Keep green for any source-pin or embedding replacement. |
| Upstream session calls, private pre-compress writes, and neutral failures | `uv run --frozen --extra dev pytest tests/test_sync_worker.py tests/test_cashew_extract.py tests/test_on_pre_compress.py -q` | Require real multi-process persistence evidence before private calls change. |
| Vector/keyword/BFS retrieval fallback | `uv run --frozen --extra dev pytest tests/test_retrieval.py tests/test_recall.py tests/test_handle_tool_call.py -q` | Preserve truthful status, context correctness, and deterministic fallback. |
| Sleep phases and cron import/reconciliation paths | `uv run --frozen --extra dev pytest tests/test_sleep_cron_script.py tests/test_sleep_cron_reconcile.py tests/test_consolidation_safety_contracts.py -q` | Preserve upstream delegation plus both loader and reconciliation paths. |
