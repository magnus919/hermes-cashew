...
def test_run_sleep_cycle_passes_edge_cap_to_cross_link_batch(small_graph, monkeypatch):
    import numpy as np
    ...
    def fake_batch(*args, **kwargs):
        captured["max_edges"] = kwargs["max_edges"]
        return {"created": 0, "skipped": 0, "same_source_skipped": 0, "capped": True}

    monkeypatch.setattr(sleep, "_batch_cross_links", fake_batch)
    monkeypatch.setattr(
        sleep,
        "_find_candidates",
        lambda ids, matrix: (
    ...
    result = sleep.run_sleep_cycle(small_graph, limit=6, max_edges=17)

    assert captured["max_edges"] == 17
