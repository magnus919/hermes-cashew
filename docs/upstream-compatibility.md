...
The selected source includes the schema fast path but does not resolve every
upstream gap. Its capped cross-link loop can stop before flushing its pending
batch, it still has no supported device-injection contract, and several wrapper
calls still target private upstream functions. The local consolidation defects
